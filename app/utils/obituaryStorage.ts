...
export function getGraveyardStats(graveyard: ShoeObituary[]): {
  ...
  if (graveyard.length === 0) return { total_shoes: 0, total_miles: 0, avg_lifespan: 0, highest_miles: null };
  ...
  return { total_shoes: graveyard.length, total_miles: Math.round(total_miles), avg_lifespan, highest_miles };
}
